...
void initialize_mpi(double* ptr_prev,double* ptr_next, double* ptr_vel,Parameters* p,size_t nbytes,int blockSize,int rank){
...
#  error "HALF_LENGTH not implemented"
	...
	for(int step=0;step</*p.nreps*/4;step++){
		...
		MPI_Sendrecv(&p.next[nowsend2up], HALF_LENGTH * p.n1 * p.n2, MPI_DOUBLE, up, 1, &p.next[nowrecvdown], HALF_LENGTH * p.n1 * p.n2, MPI_DOUBLE, down, 1, MPI_COMM_WORLD, &status);

        //更新now进程的下halo区,更新next进程的上halo区
        MPI_Sendrecv(&p.next[nowsend2down], HALF_LENGTH * p.n1 * p.n2, MPI_DOUBLE, down, 1, &p.next[nowrecvup], HALF_LENGTH * p.n1 * p.n2, MPI_DOUBLE, up, 1, MPI_COMM_WORLD, &status); //上halo区
